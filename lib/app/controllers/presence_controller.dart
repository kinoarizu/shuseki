...
import 'package:go_absensi/company_data.dart';
...
class PresenceController extends GetxController {
  ...
  Future<void> processPresence(Position position, String address, double distance) async {
    ...
    CollectionReference<Map<String, dynamic>> presenceCollection = firestore
    .collection("employee")
    .doc(uid)
    .collection("presence");
    ...
    if (snapshotPreference.docs.length == 0) {
      ...
      firstPresence(presenceCollection, todayDocId, position, address, distance, inArea);
    } else {
      ...
      if (todayDoc.exists == true) {
        ...
        if (dataPresenceToday?["keluar"] != null) {
          ...
          checkoutPresence(presenceCollection, todayDocId, position, address, distance, inArea);
        }
      } else {
        // case : not yet check in today
        checkinPresence(presenceCollection, todayDocId, position, address, distance, inArea);
      }
    }
  }
